    ...
    <section class="content">
      ...
      <div class="card">
        ...
        <table id="example1" class="table table-bordered table-striped table-hover">
                ...
                <tbody>
                @foreach ($mainitem->sons as $item)
                <tr>
                  <td>{{ $item->title }}</td>
                  {{-- <td>{{ $item->slug }}</td> --}}
                  <td>{{ $item->status_conv }}</td>                   
                  <td>    <a class="btn btn-info btn-sm" href="{{url('admin/design/editmenu', $item->id)}}">
                          ...
                          {{-- <form action="{{route('project.destroy', $item->id)}}" method="POST" class="d-inline">
                           ...
                           <td>@if($item->sons->first())
                            <a class="btn btn-info btn-sm" href="{{url('admin/design/showsubmenu', $item->id)}}">
                              <i class="fas fa-pencil-alt">
                              </i>
                             Sub menu
                          </a>
                            @endif
                            @if($item->code=='projects'||$item->code=='references')
                            <a class="btn btn-info btn-sm" href="{{url('admin/post/showbycatid', $item->id)}}">
                              <i class="fas fa-pencil-alt">
                              </i>
                            Show
                          </a>
